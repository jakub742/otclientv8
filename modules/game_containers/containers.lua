...
function refreshContainerItems(container)
  for slot = 0, container:getCapacity() - 1 do
    ...
    local rarity = item and item:getTooltip() or
    nil                                              -- Set rarity to the item's tooltip if it exists, otherwise set it to nil

    if rarity then
      local rarityLower = rarity:lower()
      if string.find(rarityLower, "legendary") then
      ...
      itemWidget:setImageSource("") -- Remove the rarity image if there is no specific rarity
    end

    itemWidget:setTooltip(rarity or "") -- Set the tooltip to the rarity if it exists, otherwise set it to an empty string
...
function toggleContainerPages(containerWindow, hasPages)
  ...
  if hasPages then
    containerWindow.miniwindowScrollBar:setMarginTop(containerWindow.miniwindowScrollBar:getMarginTop() +
      containerWindow.pagePanel:getHeight())
    containerWindow.contentsPanel:setMarginTop(containerWindow.contentsPanel:getMarginTop() +
      containerWindow.pagePanel:getHeight())
  else
    containerWindow.miniwindowScrollBar:setMarginTop(containerWindow.miniwindowScrollBar:getMarginTop() -
      containerWindow.pagePanel:getHeight())
  ...
  for _, container in pairs(g_game.getContainers()) do
    ...
    for i, child in pairs(window:getChildren()) do
      ...
      if name then
        ...
        if string.find(nameLower, "legendary") then
